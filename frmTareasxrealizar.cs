...
namespace proyecto1
{
    public partial class frmTareasxrealizar : Form
    {
        ...
        private void btnAgregarTarea_Click(object sender, EventArgs e)
        {
            ...
            frmagregarTarea.Show();
        }

        private void frmTareasxrealizar_Load(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(cadena))
                {
                    ...
                    string query = "SELECT * FROM tarea WHERE estado ='TAREA POR REALIZAR';";
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                ...
                                lstbTareasporrealizar.Items.Add(tareaCompleta);
        ...
        private void lstbTareasporrealizar_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(cadena))
                {
                    connection.Open();
                    string query = "SELECT * FROM tarea where '" + lstbTareasporrealizar.Text + "'= nombreTarea;";
                    using (SqlCommand command = new SqlCommand(query, connection))
                    {
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
        ...
        private void btnBorrarTarea_Click(object sender, EventArgs e)
        {
            try
            {
                if (MessageBox.Show("¿Estás seguro de que deseas borrar los datos?", "Advertencia", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    {
                        connection.Open();
                        string query = "Delete FROM tarea where '" + lstbTareasporrealizar.Text + "'= nombreTarea;";
